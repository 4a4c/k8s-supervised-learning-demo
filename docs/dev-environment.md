# Dev Environment: Kubernetes + Minikube

This document describes the developer environment shipped with this repo: a Dev Container with kubectl, Helm, and Minikube, plus a reliable SSH forwarding solution and UX helpers.
...
2. Start a local Kubernetes cluster:
	...
	make minikube-start
	```
	- Creates a single-node cluster using the Docker driver
...
AI/ML projects (resource-intensive workloads):
```bash
